...
# If another coroutine refreshed token very recently, skip duplicate force-refresh.
FORCE_REFRESH_DEDUP_SECONDS = 2.0
...
class KiroAuthManager:
    ...
    def __init__(
        ...
        self._last_refresh_completed_monotonic: Optional[float] = None
        ...
        logger.info(
            f"Auth manager initialized: region={region}, api_host={self._api_host}, q_host={self._q_host}"
        )
        ...
        try:
            ...
            if token_row:
                ...
                if token_data:
                    ...
                    if (
                        self._sqlite_token_key == "kirocli:social:token"
                        or token_data.get("provider")
                    ):
            ...
            if not self._is_social_login:
                for key in SQLITE_REGISTRATION_KEYS:
                    ...
                    if registration_row:
                        logger.debug(
                            f"Loaded device registration from SQLite key: {key}"
                        )
                        break
    ...
    def _load_credentials_from_file(self, file_path: str) -> None:
        ...
        try:
            ...
            if "region" in data:
                ...
                logger.info(
                    f"Region updated from credentials file: region={self._region}, api_host={self._api_host}, q_host={self._q_host}"
                )
        ...
        try:
            ...
            if self._sqlite_token_key:
                ...
                if cursor.rowcount > 0:
                    ...
                    logger.warning(
                        f"Failed to update SQLite key: {self._sqlite_token_key}, trying fallback"
                    )
    ...
    async def _refresh_token_kiro_desktop(self) -> None:
        ...
        async with httpx.AsyncClient(
            timeout=30, verify=not ALLOW_UNTRUSTED_TLS
        ) as client:
    ...
    async def get_access_token(self) -> str:
        ...
        """
        # Fast path without lock to reduce contention under concurrent traffic.
    ...
    async def force_refresh(self) -> str:
        ...
        async with self._lock:
            now = time.monotonic()
            if (
                self._last_refresh_completed_monotonic is not None
                and (now - self._last_refresh_completed_monotonic)
                <= FORCE_REFRESH_DEDUP_SECONDS
                and self._access_token
            ):
                logger.debug(
                    "Skipping duplicate force_refresh (recent refresh already completed)"
                )
                return self._access_token
            ...
            self._last_refresh_completed_monotonic = time.monotonic()
            if not self._access_token:
                raise ValueError("Failed to obtain access token after force refresh")
            return self._access_token
