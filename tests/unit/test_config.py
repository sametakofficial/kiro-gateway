...
class TestGatewayInterventionDefaults:
    """Tests for transparent-proxy intervention defaults."""

    def test_payload_limit_uses_new_bytes_key_default(self):
        """
        What it does: Verifies default KIRO_MAX_PAYLOAD_BYTES value.
        Purpose: Keep payload guard deterministic with documented default headroom.
        """
        print("Setup: Clearing payload limit env vars...")
        with patch.dict(
            os.environ,
            {
                "KIRO_MAX_PAYLOAD_BYTES": "",
                "KIRO_MAX_PAYLOAD_CHARS": "",
            },
            clear=False,
        ):
            os.environ.pop("KIRO_MAX_PAYLOAD_BYTES", None)
            os.environ.pop("KIRO_MAX_PAYLOAD_CHARS", None)
            ...
            print(f"KIRO_MAX_PAYLOAD_BYTES: {config_module.KIRO_MAX_PAYLOAD_BYTES}")
            assert config_module.KIRO_MAX_PAYLOAD_BYTES == 590000

    def test_payload_limit_supports_legacy_alias(self):
        """
        What it does: Verifies backward compatibility with legacy payload env key.
        Purpose: Existing deployments using KIRO_MAX_PAYLOAD_CHARS must keep working.
        """
        print("Setup: Using legacy KIRO_MAX_PAYLOAD_CHARS=777777...")
        with patch.dict(os.environ, {"KIRO_MAX_PAYLOAD_CHARS": "777777"}, clear=False):
            os.environ.pop("KIRO_MAX_PAYLOAD_BYTES", None)
            ...
            print(f"KIRO_MAX_PAYLOAD_BYTES: {config_module.KIRO_MAX_PAYLOAD_BYTES}")
            print(
                f"KIRO_MAX_PAYLOAD_CHARS alias: {config_module.KIRO_MAX_PAYLOAD_CHARS}"
            )
            assert config_module.KIRO_MAX_PAYLOAD_BYTES == 777777
            assert config_module.KIRO_MAX_PAYLOAD_CHARS == 777777

    def test_invasive_validators_default_on(self):
        """
        What it does: Verifies tool/message validators are ON by default.
        Purpose: Prevent common malformed-request upstream failures out of the box.
        """
        print("Setup: Clearing validator env vars...")
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TOOL_PAIRING_VALIDATOR_ENABLED", None)
            os.environ.pop("MESSAGE_STRUCTURE_VALIDATOR_ENABLED", None)
            ...
            print(
                "Validator defaults:",
                config_module.TOOL_PAIRING_VALIDATOR_ENABLED,
                config_module.MESSAGE_STRUCTURE_VALIDATOR_ENABLED,
            )
            assert config_module.TOOL_PAIRING_VALIDATOR_ENABLED is True
            assert config_module.MESSAGE_STRUCTURE_VALIDATOR_ENABLED is True

    def test_tool_size_guards_default_on(self):
        """
        What it does: Verifies tool payload/args guards are ON by default.
        Purpose: Reduce vague upstream 400 errors from oversized tool payloads.
        """
        print("Setup: Clearing tool guard env vars...")
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TOOL_RESULT_GUARD_ENABLED", None)
            os.environ.pop("TOOL_CALL_ARGS_GUARD_ENABLED", None)
            ...
            print(
                "Tool guard defaults:",
                config_module.TOOL_RESULT_GUARD_ENABLED,
                config_module.TOOL_CALL_ARGS_GUARD_ENABLED,
            )
            assert config_module.TOOL_RESULT_GUARD_ENABLED is True
            assert config_module.TOOL_CALL_ARGS_GUARD_ENABLED is True

    def test_skip_auth_acknowledged_defaults_false(self):
        """
        What it does: Verifies SKIP_AUTH_ACKNOWLEDGED defaults to false.
        Purpose: Prevent accidental auth bypass without explicit operator acknowledgment.
        """
        print("Setup: Clearing SKIP_AUTH_ACKNOWLEDGED env var...")
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SKIP_AUTH_ACKNOWLEDGED", None)
            ...
            print(f"SKIP_AUTH_ACKNOWLEDGED: {config_module.SKIP_AUTH_ACKNOWLEDGED}")
            assert config_module.SKIP_AUTH_ACKNOWLEDGED is False

    def test_skip_auth_acknowledged_accepts_true_aliases(self):
        """
        What it does: Verifies SKIP_AUTH_ACKNOWLEDGED parses true aliases.
        Purpose: Keep env parsing consistent with other boolean feature flags.
        """
        print("Setup: Setting SKIP_AUTH_ACKNOWLEDGED=yes...")
        with patch.dict(os.environ, {"SKIP_AUTH_ACKNOWLEDGED": "yes"}, clear=False):
            import importlib
            import kiro.config as config_module
            ...
            print(f"SKIP_AUTH_ACKNOWLEDGED: {config_module.SKIP_AUTH_ACKNOWLEDGED}")
            assert config_module.SKIP_AUTH_ACKNOWLEDGED is True
