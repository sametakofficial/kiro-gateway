...
class TestEnsureToolCallResultConsistency:
    """Tests for ensure_tool_call_result_consistency function."""

    def test_converts_unresolved_tool_call_before_regular_user_message(self):
        """
        What it does: Converts unresolved tool_calls to text when user continues without tool_results.
        Purpose: Prevent malformed tool flow that can trigger upstream 400 errors.
        """
        print("Setup: assistant tool_call followed by regular user text...")
        messages = [
                ...
                tool_calls=[
                    {
                        "id": "call_missing",
                        "type": "function",
                        "function": {"name": "debug_tool", "arguments": '{"x": 1}'},
                    }
                ],
            ),
            UnifiedMessage(role="user", content="continue without tool result"),
        ]

        print("Action: Normalizing tool flow...")
        result, converted = ensure_tool_call_result_consistency(messages)

        print("Verification: unresolved tool_call converted to assistant text...")
        assert converted is True
        assert result[1].tool_calls is None
        assert "[Tool: debug_tool" in result[1].content

    def test_preserves_valid_tool_call_result_sequence(self):
        """
        What it does: Keeps normal assistant tool_call -> user tool_result sequence unchanged.
        Purpose: Ensure minimal intervention on valid conversations.
        """
        print("Setup: valid tool_call + tool_result sequence...")
        messages = [
            UnifiedMessage(role="user", content="start"),
            UnifiedMessage(
                role="assistant",
        ...
        print("Verification: valid pair is preserved...")
...
class TestExtractToolUses:
    ...
    def test_guards_oversized_tool_use_input_from_content_blocks(self):
        """
        What it does: Verifies oversized Anthropic-style tool_use input is omitted.
        Purpose: Prevent huge tool_use.input blobs from reaching upstream payload.
        """
        print("Setup: Content block tool_use with oversized input...")
        content = [
            {
                "type": "tool_use",
                "id": "call_big_content",
        ...
        print("Action: Extracting tool uses with strict arg guard...")
        with patch("kiro.converters_core.TOOL_CALL_ARGS_GUARD_ENABLED", True):
            with patch("kiro.converters_core.TOOL_CALL_ARGS_MAX_CHARS", 2000):
                result = extract_tool_uses_from_message(
    ...
    def test_guards_oversized_tool_call_arguments(self):
        """
        What it does: Verifies oversized tool call arguments are replaced with omission payload.
        Purpose: Prevent upstream 400 errors from huge tool call argument blobs.
        """
        print("Setup: Oversized tool call arguments...")
        tool_calls = [
            {
                "id": "call_big_args",
                "function": {
        ...
        print("Action: Extracting tool uses with strict arg limit...")
        with patch("kiro.converters_core.TOOL_CALL_ARGS_GUARD_ENABLED", True):
            with patch("kiro.converters_core.TOOL_CALL_ARGS_MAX_CHARS", 2000):
                result = extract_tool_uses_from_message(
        ...
        print(f"Result: {result}")
        assert len(result) == 1
        assert result[0]["name"] == "debug_tool"
        assert result[0]["input"] == {}
...
class TestInjectThinkingTags:
    ...
    def test_returns_original_content_when_disabled(self):
        """
        What it does: Verifies that inject_thinking_tags always injects tags.
        Purpose: The function is a utility that always injects; caller decides when to call it.
        Note: FAKE_REASONING_ENABLED check is done at routes/policy layer, not in this function.
        """
        print("Setup: Content...")
        content = "Hello, world!"

        print("Action: Inject thinking tags (always injects)...")
        result = inject_thinking_tags(content)

        print(f"Result: {result[:100]}...")
        # Function always injects tags - the decision to call it is made at routes layer
...
class TestToolCallsToText:
    ...
    def test_omits_oversized_arguments_in_text_conversion(self):
        """
        What it does: Verifies oversized tool arguments are omitted in text conversion path.
        Purpose: Prevent huge argument blobs from being appended to message content.
        """
        print("Setup: Tool call with oversized arguments...")
        tool_calls = [
            {
                "id": "call_large",
                "type": "function",
                "function": {"name": "debug", "arguments": "A" * 8000},
            }
        ]

        print("Action: Converting to text with strict arg limit...")
        with patch("kiro.converters_core.TOOL_CALL_ARGS_GUARD_ENABLED", True):
            with patch("kiro.converters_core.TOOL_CALL_ARGS_MAX_CHARS", 2000):
                result = tool_calls_to_text(tool_calls)

        print(f"Result: '{result[:120]}...'")
        assert "tool arguments omitted by gateway" in result
...
class TestBuildKiroPayloadIssue20:
    ...
    def test_with_tools_defined_omits_oversized_historical_tool_results(self):
        """
        What it does: Verifies oversized historical tool results are omitted for recovery.
        Purpose: Prevent sticky session failures after a previous oversized tool output.
        """
        print("Setup: Oversized tool_result with tools defined...")
        huge_output = "ERROR line\n" + ("A" * 12000)
        messages = [
            UnifiedMessage(role="user", content="Run the tool"),
            UnifiedMessage(
                role="assistant",
                content="",
                tool_calls=[
            ...
            UnifiedMessage(role="user", content="What happened?"),
        ]
        ...
        print("Action: Building payload with strict tool_result guard limits...")
        with patch("kiro.converters_core.TOOL_RESULT_GUARD_ENABLED", True):
            with patch("kiro.converters_core.TOOL_RESULT_MAX_CHARS", 1800):
                with patch("kiro.converters_core.TOOL_RESULT_TOTAL_MAX_CHARS", 3000):
                    result = build_kiro_payload(
                        messages=messages,
                        system_prompt="",
                        model_id="claude-sonnet-4",
        ...
        payload = result.payload
        history = payload["conversationState"].get("history", [])
        found_omission_marker = False
        for msg in history:
            if "userInputMessage" not in msg:
                continue
            context = msg["userInputMessage"].get("userInputMessageContext", {})
            for tr in context.get("toolResults", []):
                for block in tr.get("content", []):
                    text = block.get("text", "")
                    if "historical tool output omitted by gateway" in text:
                        found_omission_marker = True

        if not found_omission_marker:
            current_context = payload["conversationState"]["currentMessage"][
                "userInputMessage"
            ].get("userInputMessageContext", {})
            for tr in current_context.get("toolResults", []):
                for block in tr.get("content", []):
                    text = block.get("text", "")
                    if "historical tool output omitted by gateway" in text:
                        found_omission_marker = True
        assert found_omission_marker is True

    def test_tool_result_guard_enforces_total_budget(self):
        """
        What it does: Verifies aggregate budget is enforced across multiple tool results.
        Purpose: Ensure the request-wide tool_result size cap is respected.
        """
        print("Setup: Multiple oversized tool_results...")
        huge_a = "A" * 9000
        huge_b = "B" * 9000

        messages = [
            UnifiedMessage(role="user", content="Run tool A"),
            UnifiedMessage(
                role="assistant",
                content="",
                tool_calls=[
            ...
            UnifiedMessage(role="assistant", content="Next step"),
            UnifiedMessage(role="user", content="Run tool B"),
            UnifiedMessage(
                role="assistant",
                content="",
                tool_calls=[
            ...
            UnifiedMessage(role="user", content="Summarize"),
        ]
        ...
        print(
            "Action: Building payload with low total budget (should recover by omitting history)..."
        )
        with patch("kiro.converters_core.TOOL_RESULT_GUARD_ENABLED", True):
            with patch("kiro.converters_core.TOOL_RESULT_MAX_CHARS", 10000):
                with patch("kiro.converters_core.TOOL_RESULT_TOTAL_MAX_CHARS", 4500):
                    result = build_kiro_payload(
                        messages=messages,
                        system_prompt="",
                        model_id="claude-sonnet-4",
    ...
    def test_tool_result_guard_converts_oversized_active_to_tool_error(self):
        """
        What it does: Verifies oversized active tool outputs are converted to tool error results.
        Purpose: Ensure the model gets a tool-level error signal instead of surfacing gateway 400 to user.
        """
        print("Setup: Oversized tool_result...")
        huge_output = "X" * 7000
        messages = [
            UnifiedMessage(role="user", content="Run"),
            UnifiedMessage(
                role="assistant",
                content="",
                tool_calls=[
        ...
        print("Action: Building payload (should convert to synthetic tool error)...")
        with patch("kiro.converters_core.TOOL_RESULT_GUARD_ENABLED", True):
            with patch("kiro.converters_core.TOOL_RESULT_MAX_CHARS", 2000):
                result = build_kiro_payload(
                    messages=messages,
                    system_prompt="",
                    model_id="claude-sonnet-4",
        ...
        for msg in history:
            ...
            for tr in context.get("toolResults", []):
                if tr.get("status") == "error":
                    for block in tr.get("content", []):
                        text = block.get("text", "")
                        if "[Tool output omitted by gateway]" in text:
                            found_tool_error = True

        if not found_tool_error:
            current_context = payload["conversationState"]["currentMessage"][
                "userInputMessage"
            ].get("userInputMessageContext", {})
            for tr in current_context.get("toolResults", []):
        ...
        assert found_tool_error is True

    def test_tool_result_guard_never_raises_user_facing_guard_error(self):
        """
        What it does: Verifies oversized active tool output is handled without ValueError.
        Purpose: Ensure guard errors are converted to tool-level errors, not user-facing 400.
        """
        print("Setup: Oversized tool_result...")
        huge_output = "X" * 7000
        messages = [
        ...
        print("Action: Building payload (should not raise ValueError)...")
        with patch("kiro.converters_core.TOOL_RESULT_GUARD_ENABLED", True):
            with patch("kiro.converters_core.TOOL_RESULT_MAX_CHARS", 2000):
                result = build_kiro_payload(
                    messages=messages,
    ...
    def test_tool_result_guard_allows_within_limits(self):
        """
        What it does: Verifies tool results within limits pass through unchanged.
        Purpose: Ensure guard doesn't interfere with normal-sized outputs.
        """
        print("Setup: Normal-sized tool_result...")
        normal_output = "Result: OK\nStatus: Success"
        messages = [
            UnifiedMessage(role="user", content="Run"),
            UnifiedMessage(
                role="assistant",
        ...
        print("Action: Building payload (should succeed)...")
        with patch("kiro.converters_core.TOOL_RESULT_GUARD_ENABLED", True):
            with patch("kiro.converters_core.TOOL_RESULT_MAX_CHARS", 50000):
                result = build_kiro_payload(
                    messages=messages,
                    system_prompt="",
                    model_id="claude-sonnet-4",
...
class TestQueuedAnnounceGuard:
    """Tests for queued-announce payload compaction guard."""

    def test_compacts_oversized_queued_announce_text_blocks(self):
        """
        What it does: Verifies oversized queued announce blocks are compacted.
        Purpose: Prevent upstream vague 400 errors from huge busy-queue notices.
        """
        print("Setup: Oversized queued-announce user message block...")
        oversized_notice = (
            "[Tue 2026-02-10 18:52 UTC] [Queued announce messages while agent was busy]\n\n"
            "---\n"
            "Queued #1\n"
            'A background task "stress-london" just completed successfully.\n\n'
            "Findings:\n" + ("L" * 6000)
        )
        ...
        messages = [
            ...
            UnifiedMessage(
                role="user",
                content=[{"type": "text", "text": oversized_notice}],
            ),
            UnifiedMessage(role="assistant", content="continue"),
            UnifiedMessage(role="user", content="final question"),
        ]

        print("Action: Building payload with queued-announce guard enabled...")
        with patch("kiro.converters_core.QUEUED_ANNOUNCE_GUARD_ENABLED", True):
            with patch("kiro.converters_core.QUEUED_ANNOUNCE_MAX_CHARS", 1400):
                with patch("kiro.converters_core.QUEUED_ANNOUNCE_HEAD_CHARS", 700):
                    with patch("kiro.converters_core.QUEUED_ANNOUNCE_TAIL_CHARS", 300):
                        result = build_kiro_payload(
                            messages=messages,
                            system_prompt="",
        ...
        print("Checking compaction notice marker in history...")
        assert "[Gateway notice] Queued-announce findings truncated" in history_text

    def test_does_not_compact_regular_user_messages(self):
        """
        What it does: Verifies non-queued user messages are untouched.
        Purpose: Preserve transparency and avoid modifying normal user prompts.
        """
        print("Setup: Long regular user prompt (not queued announce)...")
        regular_text = "R" * 5000

        messages = [UnifiedMessage(role="user", content=regular_text)]

        print("Action: Building payload with strict queued-announce threshold...")
        with patch("kiro.converters_core.QUEUED_ANNOUNCE_GUARD_ENABLED", True):
            with patch("kiro.converters_core.QUEUED_ANNOUNCE_MAX_CHARS", 1000):
                result = build_kiro_payload(
                    messages=messages,
                    system_prompt="",
                    model_id="claude-opus-4.6",
        ...
        current_content = result.payload["conversationState"]["currentMessage"][
            "userInputMessage"
        ]["content"]

        print("Checking regular prompt remained untouched...")
        assert (
            "[Gateway notice] Queued-announce findings truncated" not in current_content
        )
        assert regular_text in current_content

    def test_guard_can_be_disabled(self):
        """
        What it does: Verifies queued-announce compaction can be fully disabled.
        Purpose: Ensure user control over optional gateway behavior.
        """
        print("Setup: Oversized queued-announce with guard disabled...")
        oversized_notice = (
            "[Queued announce messages while agent was busy]\n\n"
            'A background task "stress-paris" just completed successfully.\n\n'
            "Findings:\n" + ("P" * 4500)
        )
        ...
        print("Action: Applying guard directly with feature disabled...")
        with patch("kiro.converters_core.QUEUED_ANNOUNCE_GUARD_ENABLED", False):
            guarded = apply_queued_announce_guard(messages)

        print("Checking message stayed unchanged...")
        block_text = guarded[0].content[0]["text"]
        assert "[Gateway notice] Queued-announce findings truncated" not in block_text
        assert block_text == oversized_notice
