...
class TestKiroHttpClientRequestWithRetry:
    ...
    @pytest.mark.asyncio
    async def test_skip_auth_does_not_request_token(self, mock_auth_manager_for_http):
        """
        What it does: Verifies SKIP_AUTH mode does not call get_access_token().
        Purpose: Ensure direct proxy mode bypasses upstream bearer auth.
        """
        print("Setup: Creating KiroHttpClient in SKIP_AUTH mode...")
        http_client = KiroHttpClient(mock_auth_manager_for_http)
        ...
        print("Action: Executing request with SKIP_AUTH=True...")
        with patch.object(http_client, "_get_client", return_value=mock_client):
            with patch("kiro.http_client.SKIP_AUTH", True):
                with patch(
                    "kiro.http_client.get_kiro_headers",
                    return_value={"Authorization": "Bearer x", "x-test": "1"},
                ):
                    response = await http_client.request_with_retry(
                        "POST", "https://api.example.com/test", {"data": "value"}
                    )

        print("Verification: get_access_token was not called...")
        mock_auth_manager_for_http.get_access_token.assert_not_called()
        mock_client.request.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_skip_auth_does_not_force_refresh_on_403(
        self, mock_auth_manager_for_http
    ):
        """
        What it does: Verifies SKIP_AUTH mode does not call force_refresh() on 403.
        Purpose: Ensure gateway passes through upstream auth behavior in bypass mode.
        """
        print("Setup: Creating KiroHttpClient in SKIP_AUTH mode...")
        http_client = KiroHttpClient(mock_auth_manager_for_http)
        ...
        print("Action: Executing request with SKIP_AUTH=True and 403 response...")
        with patch.object(http_client, "_get_client", return_value=mock_client):
            with patch("kiro.http_client.SKIP_AUTH", True):
                with patch("kiro.http_client.get_kiro_headers", return_value={}):
                    response = await http_client.request_with_retry(
        ...
        print("Verification: force_refresh was not called in SKIP_AUTH mode...")
