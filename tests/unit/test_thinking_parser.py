...
class TestInjectThinkingTags:
    ...
    def test_no_injection_when_disabled(self):
        """
        What it does: Verifies inject_thinking_tags always injects tags when called.
        Purpose: The function is a utility; caller decides when to call based on policy.
        Note: FAKE_REASONING_ENABLED check is done at routes/policy layer, not here.
        """
        print("Testing tag injection (always injects when called)...")
        from kiro.converters_core import inject_thinking_tags

        result = inject_thinking_tags("Hello")

        print(f"Result: '{result[:50]}...'")
        # Function always injects - decision to call is made at routes layer
        assert "<thinking_mode>enabled</thinking_mode>" in result
        assert "Hello" in result
